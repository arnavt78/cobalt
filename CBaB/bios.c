...
int bios() {
  ...
  fflush(stdout);          // Flushes
  verbose("Completed 3 Prints and 2 Flushes.");
  sleep(delay); // Waits 3 Seconds.
  printf("\e[1;1H\e[2J"); // Clears Screen.
  int betabuild = -1;     // Null
  delay = 1;
  verbose("Betabuild has been declared as -1 [Null].");
  switch (ver) {                // Simpler Version Management
  case 100:        // If the Version is 100,
    verbose("Betabuild has been changed. [-1 --> 0]");
    betabuild = 0; // Set Betabuild to False
    printf("%s", os);
    break;                                                                                                                                  // Exits
  default:                                                                                                                                  // If Version is NOT 100,
    betabuild = 1;
    verbose("Betabuild has been changed. [-1 --> 1]");                                                                                                                          // Set Betabuild to True
    printf("%s", os);
    verbose("Current Build has been marked as beta. Opening Balloon...");                                                                                                                       // Prints OS
  ...
  sleep(delay);
  verbose("Checking to see if Betabuild was changed successfully...");
  ...
  case -1:
    verbose("Betabuild was not changed.");   // If BetaBuild is -1 (Should not be -1, if it is prints an Error.)
    printf("\e[1;1H\e[2J");    // Clears SCreen
    printf("%s", os);          // Prints OS Name
  ...
  verbose("Check completed. [Successful]");
  printf("\n>> Starting Services... ");
  sleep(delay);
  services();
}